...
public class GameViewSize
{
    ...
    public static EditorWindow GetMainGameView()
    {
        ...
        Type T2 = Type.GetType("UnityEditor.WindowLayout,UnityEditor");
        MethodInfo GetMainGameView = T2.GetMethod("FindEditorWindowOfType", BindingFlags.NonPublic | BindingFlags.Static);
        System.Object Res = GetMainGameView.Invoke(null, new object[] { T });
    ...
    public static void GetGameRenderSize(out int width, out int height)
    {
        ...
        width = (int)UnityEditor.Handles.GetMainGameViewSize().x;// size.x;
        height = (int)UnityEditor.Handles.GetMainGameViewSize().y;// size.y;
    }
    ...
    {
        var sizeObj = FindRecorderSizeObj(width, height);
        if (sizeObj != null)
        {
    ...
    static object FindRecorderSizeObj(int width, int height)
    {
        var group = Group();
        ...
        while (itr.MoveNext())
        {
            ...
            if (txt == "(Recording resolution)" || txt.Contains(string.Format("{0}x{1}", width, height)))
                return itr.Current;
        }
    ...
    static object NewSizeObj(int width, int height)
    {
        ...
        var sizeObj = c.Invoke(new object[] { 1, width, height, string.Format("(R) {0}x{1}", width, height) });
        return sizeObj;
    }
